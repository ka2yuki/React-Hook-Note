...
JavaScript はクロージャをサポートしているため  
内側の関数（例：handleClick）は外側の関数（例：Board）で定義されている変数や関数にアクセスできます。  
handleClick 関数は、state である squares を読み取ったり、setSquares メソッドを呼び出したりできます。  
